...
void test_validate_my_username()
{
    ...
    // Step 1: Call the my_username() function to get the hardcoded username
    
    const char *hardcoded_username = my_username();

    // Step 2: Obtain the username from the configuration file using malloc_username_from_conf_file()
    const char *conf_username = malloc_username_from_conf_file();

    // Step 3: Use the Unity assertion TEST_ASSERT_EQUAL_STRING_MESSAGE to verify the two strings are equal
    TEST_ASSERT_EQUAL_STRING_MESSAGE(hardcoded_username, conf_username, "The usernames do not match!");

    // Free the allocated memory after comparison
